...
//}
  ...
  void SimulateController::SeUserMotionCalcRollSpeed(SeUserVelocity tVel,
                                                     short *tLeftSpeed,
                                                     short *tRightSpeed)
  {
  ...
  SeUserOdometry SimulateController::SeUserOdometryCalculate(
      int nLeftEncoderDelta, int nRightEncoderDelta, unsigned int nDeltaTime)
  {
  ...
  void SimulateController::initialize()
  {
    ...
    fBaseTicksPerMeter = (fBaseEncoderResolution * fBaseGearReduction) / (fBaseWheelDiameter * M_PI);
    nCntlDuration = 33;
    ...
    printf("simulate controller is initialized...");
  }

  void SimulateController::run()
  {
    running = true;
    printf("simulate controller is running...");
    simulate_controller_thread = boost::thread(
        boost::bind(&SimulateController::motionCallback, this));

  }

  void SimulateController::quit()
  {
    printf("simulate controller is quitting!");
    running = false;

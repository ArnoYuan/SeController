...
// update filter
  bool OdomEstimation::update(const Time& filter_time)
  {
    ...
    if(dt < 0)
    {
      printf("Will not update robot pose with time %f sec in the past.\n", dt);
      return false;
    }
  ...
  void OdomEstimation::addMeasurement(const std::string& meas_type,
  ...
  void OdomEstimation::addMeasurement(
      const std::string& meas_type, const Transform& meas,
      const MatrixWrapper::SymmetricMatrix& covar)
  {
    ...
    for(unsigned int i = 0; i < covar.rows(); i++)
    {
      if(covar(i + 1, i + 1) == 0)
      {
        printf("Covariance specified for measurement %s is zero\n",
               meas_type.c_str());
        return;
      }
    }
